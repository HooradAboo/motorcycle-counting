...
def run(
    ...
    dt, seen, frame_idx = [0.0, 0.0, 0.0], 0, 0

    # DeepSORT -> Add all IDs to a set to calculate the total number of tracked objects
    object_ids = {''}
            ...
            if len(det):
                ...
                for track in tracker.tracks:
                    ...
                    with open(txt_path + '.txt', 'a') as f:
                        line = save_format.format(frame=frame_idx, id=track.track_id, x1=int(bbox[0]), y1=int(
                            bbox[1]), w=int(bbox[2] - bbox[0]), h=int(bbox[3] - bbox[1]), x=-1, y=-1, z=-1)
                        f.write(line)
    ...
    LOGGER.info(f'{len(object_ids)-1} objects detected.')
